...
Scheduler::Scheduler(int threadPoolSize, TickIntervalsE tickInterval = TickIntervalsE::Interval_040ms) :
...
void Scheduler::execute() {
    while (true) {
        ...
        while(this->executionStatusQueue.empty() == false) {
            ...
            if (auto it = taskLookupTable.find(std::get<0>(statusTuple)); it != taskLookupTable.end()) {
                it->second->setStatus(std::get<1>(statusTuple), std::get<2>(statusTuple));
            }
        }
...
TaskID Scheduler::addFixedRateTask(std::function<void()> functor, std::chrono::milliseconds interval) {
    return this->addTask(Task::TypeT::FixedRate, std::move(functor), interval);
}

TaskID Scheduler::addFixedDelayTask(std::function<void()> functor, std::chrono::milliseconds interval) {
    return this->addTask(Task::TypeT::FixedDelay, std::move(functor), interval);
}

TaskID Scheduler::addTask(Task::TypeT type, std::function<void()> functor, std::chrono::milliseconds interval) {
    auto tid = this->getNewTaskID();

    this->taskLookupTable[tid] = std::make_unique<Task>(
            Task(tid,
                 type,
                 std::move(functor),
                 interval)
